...
Future<Uint8List> quotation(SaleModel invoice, type, {print = false}) async {
  final pdf = Document();
  pdf.addPage(
    Page(
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(invoice.shopId!.name!.capitalize!,
                      ...
                      if (invoice.shopId?.contact?.isNotEmpty == true)
                        Text(" ${invoice.shopId?.contact ?? ""}",
                            style: const TextStyle(fontSize: 14)),
                      if (invoice.shopId?.receiptemail?.isNotEmpty == true)
                        Text(" ${invoice.shopId?.receiptemail ?? ""}",
                            style: const TextStyle(fontSize: 14)),
              ...
              if (invoice.attendant != null)
                Text("Prepared by : ${invoice.attendant!.username ?? ""}"),
              Spacer(),
              Align(
