...
**How to apply:** Serialize preflight, write, and verification per sale when possible. For legacy updates, omit create-only line inventory because Pointify does not round-trip it, but send the original product→inventory mapping as a proxy-only precondition and verify it against the fresh readback. Still verify product, quantity, price, tax, discounts, totals, references, and status. If a stale retry exactly matches the authoritative sale, treat it as an idempotent success without writing again. Treat an in-process lock only as a local race guard; cross-instance and direct-upstream writers still require atomic revision-and-status enforcement at the authoritative backend.