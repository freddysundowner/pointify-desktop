...
## M-Pesa has NO credit — partial = accumulate
There is no credit/balance/customer-owed concept for M-Pesa. A short payment is handled by **pay-until-enough**: the cashier claims MULTIPLE recent Till payments whose amounts SUM to the sale total (running tally; sale completes only when sum ≥ total), OR switches to **Split** to cover the remainder in cash/bank. Do NOT route M-Pesa shortfalls through payCredit/Credit. **Why:** the till money is fixed per payment and can't be a debt. **How to apply:** pure-M-Pesa selection accumulates into a list → `amountPaid` = sum, `mpesaTransId` = all refs joined. Upstream sale-finalize/allocation must therefore accept and allocate MULTIPLE M-Pesa codes for one sale (comma-joined or an array), marking each allocated — single-code allocation breaks the multi-payment case.
